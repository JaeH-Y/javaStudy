...
//	- 축구공: 3000원
	...
	public static void main(String[] args) {
		...
		// 복사본 생성
		Set<Equipment> storage3 = new HashSet<Equipment>(storage1);
		
		System.out.println("*** 합집합 ***");
		storage3.addAll(storage2);
		Iterator<Equipment> iter = storage3.iterator();
		
		while(iter.hasNext()) {
			System.out.println(iter.next());
		}
		
		storage3 = new HashSet<Equipment>(storage1);
		System.out.println("*** 교집합 ***");
		storage3.retainAll(storage2);
		iter = storage3.iterator();
		while(iter.hasNext()) {
			System.out.println(iter.next());
		}
